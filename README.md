...
With a `meta` object, `ggforest()` reads everything it needs directly:
...
- **Table columns** — add `meta::forest()`-style effect / CI / weight
  columns with `ggforest(columns = TRUE)`, or custom columns with
  `geom_forest_text()`.
